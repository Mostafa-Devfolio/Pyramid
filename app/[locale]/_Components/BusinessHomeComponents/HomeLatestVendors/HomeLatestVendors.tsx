'use client';
import { IHomeLatestVendors } from '@/app/[locale]/interface/homeLatestVendors';
...
export default function HomeLatestVendors({ mainType }: mainType) {
  ...
  useEffect(() => {
    async function homeLatestVendors() {
      ...
      const response = await fetch(
        `${baseURL2}vendors?filters[businessType][slug][$eq]=${mainType}&filters[isActive][$eq]=true&sort[0]=createdAt:desc&pagination[page]=1&pagination[pageSize]=8&populate=logo`,
        {
          method: 'get',
        }
      ).then((res) => res.json());
  ...
  return (
    <>
      <Button variant={'outline'} onClick={() => setIsGrid((prev) => !prev)}>
      ...
      <div className={`grid gap-4 ${isGrid ? 'sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4' : 'grid-cols-1'}`}>
        {isLoading ? (
          ...
          vendors.map((vendor: IHomeLatestVendors) => {
