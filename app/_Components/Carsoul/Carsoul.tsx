...
export async function CarouselHome({typee, mainType}: Type) {
      ...
      const query: string[] = [];
      if(typee){query.push(`filters[placement][$eq]=${typee}`)};
      if(mainType){query.push(`filters[businessType][slug][$eq]=${mainType}`)};
  ...
  return (<>
    {bannerData.length>0 && <Carousel className="w-full">
      <CarouselContent>
          ...
          return <CarouselItem key={banner.id}>
                  ...
                  <Card className="overflow-hidden">
                    <CardContent className="flex aspect-[3/2] h-64 items-center justify-center">
                      <Image width={500} height={500} className="w-full object-cover" src={banner.image.url == null ? IMAGE_PLACEHOLDER : `http://localhost:1337${banner.image.url}`} alt={banner.image?.alternativeText}/>
                    </CardContent>
                  </Card>
            </div>
          </CarouselItem>
