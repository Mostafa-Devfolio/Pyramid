...
export default async function HomeTopRatedVendors({mainType}: mainType) {
  ...
  async function topRatedVendors(){
    const response = await fetch(`${baseURL2}vendors?filters[businessType][slug][$eq]=${mainType}&filters[isActive][$eq]=true&sort[0]=rating:desc&pagination[page]=1&pagination[pageSize]=8&populate=logo`,{
      method: 'get'
    }).then((res) => res.json());
