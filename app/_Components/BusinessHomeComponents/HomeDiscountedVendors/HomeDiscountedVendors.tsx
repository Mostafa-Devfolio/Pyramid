...
export default function HomeDiscountedVendors({mainType} :{mainType: string}) {
    ...
    useEffect(()=>{
        async function getDiscountedVendorsProducts(){
            setIsLoading(true);
            const response = await fetch(`${baseURL2}business-types/${mainType}/vendors/discounted?minDiscountedProducts=20&pagination[page]=1&pagination[pageSize]=8&populate=*`,{
                method: 'get',
            }).then((res)=> res.json());
            setIsLoading(false);
    ...
    return (
        <div>
            <Button onClick={()=> setIsGrid(prev => !prev)}><i className="fa-solid fa-grip"></i></Button>
            <div className={`grid gap-4 ${isGrid ? 'lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-1' : 'grid-cols-1'}`}>
                {isLoading ? <VendorsSkeleton /> : vendors.map((vendor: IDiscountedVendorsHome) => {
