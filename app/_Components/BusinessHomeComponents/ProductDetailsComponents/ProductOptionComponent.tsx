...
export default function ProductOptionComponent({products}: optiongroup) {
    ...
    <div className="flex gap-3 mt-3">
        <div className='border rounded-2xl flex items-center gap-2'>
            <div className="">
                {(count == null || count == 0) && !(getCar?.items?.some((ite:any) => products.id == ite.product.id))  ? <Button onClick={() => {setCount(count+1); addItemToCart(); addToCartt(products.id)}}>Add To Cart</Button> : 
                    <div className="flex items-center gap-3">
                        {auth ? getCar?.items?.map((itemm: any) => products.id == itemm.product.id && <Button key={itemm.id} onClick={() => {changeCart(itemm.id, itemm.quantity-1)}}>-</Button>) : <Button onClick={() => {decrease(); addItemToCart(); addToCartt(products.id)}}>-</Button>}
                        {auth ? getCar?.items?.map((itemm: any) => products.id == itemm.product.id && <p key={itemm.id}>{itemm.quantity}</p>) : <p>{count}</p> }
                        {auth ? getCar?.items?.map((itemm: any) => products.id == itemm.product.id && <Button key={itemm.id} onClick={() => {changeCart(itemm.id, itemm.quantity+1)}}>+</Button>) : <Button onClick={() => {increase(); addItemToCart(); addToCartt(products.id)}}>+</Button>}
                    </div>
                }
            </div>
        </div>
    ...
    <div className="my-7">
        {(auth && cartPrice != null) ? <h4>Total: {cartPrice}</h4>
