...
export default async function HomeAllVendors({mainType}: mainType) {
    ...
    async function homeAllVendors(){
        const response = await fetch(`${baseURL2}vendors?filters[businessType][slug][$eq]=${mainType}&filters[isActive][$eq]=true&pagination[page]=1&pagination[pageSize]=20&populate=logo`,{
            method: 'get',
        }).then((res)=> res.json());
